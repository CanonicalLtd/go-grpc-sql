...
func (c *Conn) Exec(query string, args []driver.Value) (driver.Result, error) {
	...
	response, err := c.exec(legacy.NewRequestConnExec(query, values))
	if err != nil {
		return nil, err
	}
...
func (c *Conn) Close() error {
	if _, err := c.exec(legacy.NewRequestClose()); err != nil {
		return err
	}
...
func (c *Conn) Begin() (driver.Tx, error) {
	response, err := c.exec(legacy.NewRequestBegin())
	if err != nil {
		return nil, err
	}
	tx := &Tx{
...
func (c *Conn) exec(request *legacy.Request) (*legacy.Response, error) {
	if c.grpcConnDoomed {
	...
	case legacy.RequestCode_SQLITE_ERROR:
